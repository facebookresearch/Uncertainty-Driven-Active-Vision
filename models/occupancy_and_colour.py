...
class model(nn.Module):
    ...
    def embedding_to_values(
        self, embedding, points, matricies, params, target_params, num_imgs
    ):
        # remove unused image parameters
        matricies = matricies[:, :num_imgs]
        params = params[:, :num_imgs]
        embedding = [e[:num_imgs] for e in embedding]

        # get direction of camera
        direction = points - target_params[:, None, :3]
        direction = direction / direction.pow(2).sum(-1).sqrt().unsqueeze(-1)
        ...
        occ, features = self.merger.occupancy(occ)

        # get target camera paramater positional embedding and concatenate with older information
        target_params_embedding = (
            self.positional_encoding(target_params)
            .unsqueeze(1)
            .repeat(1, pos_embedding.shape[1], 1)
        )
        all_features = torch.cat(
            (occ, features, orig_points, direction, target_params_embedding), dim=-1
        )

        # predict colour
        colour = self.merger.colour(all_features)
        pred = torch.cat((occ, colour), dim=-1)

        return pred

    # predict only occupancy from image embedding
    def embedding_to_occ(self, embedding, points, matricies, params, num_imgs):
    ...
    def forward(
        self, imgs, points, matricies, params, target_params, num_imgs, train=True
    ):
        imgs = imgs[:, :num_imgs]
        matricies = matricies[:, :num_imgs]
        params = params[:, :num_imgs]
...
class Merger(nn.Module):
    def __init__(self, options):
        ...
        # layers for colour prediction
        layers = []
        layers.append(nn.Linear(154 + hidden_size, hidden_size))
        layers.append(nn.Linear(hidden_size, hidden_size))
        layers.append(nn.Linear(hidden_size, hidden_size))
        layers.append(nn.Linear(hidden_size, 3))
        self.color_layers = nn.ModuleList(layers)
    ...
    def occupancy(self, occ):
        ...
        out = self.fc_out(self.actvn(net).view(occ.shape[0], occ.shape[2], -1))
        return out, net

    # colour predictions
    def colour(self, features):
        for e, l in enumerate(self.color_layers):
            features = l(features)
            if e < len(self.color_layers) - 1:
